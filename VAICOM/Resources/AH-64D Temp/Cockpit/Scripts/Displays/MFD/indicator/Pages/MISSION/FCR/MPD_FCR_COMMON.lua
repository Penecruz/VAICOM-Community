...
-- Heading Scale -----------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------------
local HeadingScaleAreaPH = addPlaceholder("HeadingScaleAreaPH", {0, 400}, VideoAreaPH.name, {{"COMMON_HeadingShow"}})
...
addBigText("HeadingDigits", "000", {0, HeadingScaleNotchLen + 7}, "CenterBottom", HeadingScaleAreaPH.name, {{"COMMON_HeadingDigits"}})
...
-- notches and marks
local HeadingScalePH = addPlaceholder("HeadingScalePH", {0, 0}, HeadingScaleMasksPH.name, {{"COMMON_HeadingScale", HeadingScaleNotchSpace * 18}})
...
local fcr_centerline_bearing = addPointer_FcrCenterlineBearing("FCR_CenterlineBearing", {0, HeadingScalePointers_y}, HeadingScaleMasksPH.name, {{"COMMON_FcrCenterlineBearing", HeadingScaleNotchSpace * 18}})
setElemsClipLevel(fcr_centerline_bearing, 1)

local alternate_sensor_bearing = addPointer_AlternateSensorBearing("AlternateSensorBearing", {0, HeadingScalePointers_y}, HeadingScaleMasksPH.name, {{"COMMON_AlternateSensorBearing", HeadingScaleNotchSpace * 18}})
setElemsClipLevel(alternate_sensor_bearing, 1)

local adf_bearing = addPointer_AdfBearing("ADF_Bearing", {0, HeadingScalePointers_y}, HeadingScaleMasksPH.name, {{"COMMON_AdfBearing", HeadingScaleNotchSpace * 18}})
setElemsClipLevel(adf_bearing, 1)

local bobup_heading = addPointer_BobUpHeading("CommonOrBobUpHeading", {0, HeadingScalePointers_y}, HeadingScaleMasksPH.name, {{"COMMON_CommonOrBobUpHeading", HeadingScaleNotchSpace * 18}})
...
local FOR_PH = addPlaceholder("FOR_PH", {0, FOR_y + FOR_h05 + FOR_margin_bottom}, VideoAreaPH.name, {{"DSPLS_FCR_DisableFOR", 2}})
...
local FOR_PNVS_PH = addPlaceholder("FOR_PNVS_PH", {0, 0}, FOR_PH.name, {{"COMMON_FOR_Type", 1}})
...
local FOR_TADS_PH = addPlaceholder("FOR_TADS_PH", {0, 0}, FOR_PH.name, {{"COMMON_FOR_Type", 2}})
...
--
local FCR_CurrentCenterline = 	addFcrCurrentCenterline("FCR_CurrentCenterline", 	{0, 0}, FOR_PH.name, {{"COMMON_FOR_FcrCurrentCenterline", FOR_w05}})
setElemsClipLevel(FCR_CurrentCenterline, 1)

local FCR_LastCenterline = 		addFcrLastCenterline("FCR_LastCenterline", 			{0, 0}, FOR_PH.name, {{"COMMON_FOR_FcrLastCenterline", 	FOR_w05}})
setElemsClipLevel(FCR_LastCenterline, 1)

-- FOV Box
local fov_box = 				addRect("FOV_Box", FOV_w, FOV_h, {0, 0}, "CenterCenter", 5, FOR_PH.name, {{"COMMON_FOR_FovPos", FOR_w05, FOR_h05}})
setElemsClipLevel(fov_box, 1)

local CuedLosDot = 				addDot("Cued_LOS_Dot", 			 {0, 0}, 					FOR_PH.name, {{"COMMON_FOR_CuedLosDotPos", FOR_w05, FOR_h05}})
...
addTextCom("SIGHT STATUS", 	"SIGHT STATUS", 	{-HAD_W1,	HAD_L1},	"RightBottom", HAD_PH.name, {{"COMMON_HAD_SightStatus"}},											HAD_SightStatusMsgs, nil, false)
addTextCom("XXXX", 			"XXXX", 			{-HAD_W1,	HAD_L2},	"RightBottom", HAD_PH.name, {{"COMMON_HAD_RangeAndRangeSource"}},									HAD_RangeAndRangeSourceStatusMsgs, nil, false)
addTextCom("C GUN", 		"C GUN", 			{ HAD_W1,	HAD_L2},	"LeftBottom",  HAD_PH.name, {{"COMMON_HAD_WeaponControl"}},											HAD_WeaponControlStatusMsgs, nil, false)
addTextCom("WEAPON STATUS",	"WEAPON STATUS", 	{ HAD_W1,	HAD_L1},	"LeftBottom",  HAD_PH.name, {{"COMMON_HAD_WeaponStatus"}},											HAD_WeaponStatusMsgs, nil, false)
addText	  ("WeaponInhibit", "WEAPON INHIB",		{ 0,		HAD_L3},	"CenterBottom",HAD_PH.name,	{{"COMMON_HAD_WeaponInhibit"}, {"DSPLS_COMMON_ColorWhite"}}, 			HAD_WeaponInhibitStatusMsgs)
addTextCom("SIGHTL", 		"SIGHTL", 			{-HAD_W2,	HAD_L2},	"RightBottom", HAD_PH.name, {{"COMMON_HAD_SightSelectStatus"}},										HAD_SightSelectStatusMsgs, nil, false)
addTextCom("TADS", 			"TADS", 			{ HAD_W2,	HAD_L2},	"LeftBottom",  HAD_PH.name, {{"COMMON_HAD_AcquisitionSelectStatus"}},								HAD_AcquisitionStatusMsgs, nil, false)

addRoundedBox("rightBoxHADFormat",  {285, 50},	"CenterCenter", {310, 90}, 10, 6, HAD_PH.name)
addRoundedBox("leftBoxHADFormat",   {-285, 50},	"CenterCenter", {310, 90}, 10, 6, HAD_PH.name)
----------------------------------------------------------------------------------------------------------------------------------
-- Common Symbology --------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------------	elem.BackgroundMaterial	= fon_mat

-------Elevation Altitude Status Window-------
local MSL = addPlaceholder("MSL_PH", {0, FOR_y + FOR_h05 + FOR_margin_bottom}, VideoAreaPH.name, {{"DSPLS_FCR_DisableFOR", 1}})
addRoundedBox("centeBoxMSL",   {-3, 5},	"CenterCenter", {160, 90}, 10, 6, MSL.name)
addTextCom("UpperLimit", 		"0", 			{70,  20},	"RightCenter", 	MSL.name, {{"DSPLS_FCR_ATM_UpperLimit"}},	nil, nil, false)
addTextCom("LowerLimit", 		"0", 			{70, -20},	"RightCenter",  MSL.name, {{"DSPLS_FCR_ATM_LowerLimit"}},	nil, nil, false)