...
local VideoAreaPH = addPlaceholder("VideoAreaCenterPHtpm", {-50, 35}, OnOffPageLINE.name, nil)
...
local function createVertsForBackground(verts_r, verts_dn)
   ...
   local offsetX, offsetLine = 3, info_box_line_width / 2
	...
	verts_out[1][1][1] = -display_size_pix_05 - 1300
	...
	verts_out[4][2][2] = display_size_pix_05 - 610
...
local pos_up, pos_dn = pb_props[pb.L1].pos, { pb_props[pb.L1].pos[1], pb_props[pb.L4].pos[2]}
...
local caption_space =  tp.height / 4
verts_r[1][3][2] = pos_caption[2] + caption_space - 35
...
-- draw caption																		
for i = 1, #caption do
	addText( string.sub(caption, i, i).."\n", {pos_caption[1], pos_caption[2] - 30 }, tp, nil, nil, nil, nil, VideoAreaPH.name, h_clip_relations.REWRITE_LEVEL, DEFAULT_LEVEL )
	pos_caption = {pos_caption[1], pos_caption[2] - tp.height + 4 }
end
