...
local VideoAreaPH = addPlaceholder("VideoAreaCenterPH", {35, 35}, OnOffPageACQ.name, nil)
...
local pos_up, pos_dn = { pb_props[pb.R1].pos[1], pb_props[pb.R1].pos[2] + 10 }, { pb_props[pb.R6].pos[1], pb_props[pb.R6].pos[2] - 75 }
...
-- draw upper line
draw_line( verts_r[1], tp.material, VideoAreaPH.name, nil, nil, nil, h_clip_relations.COMPARE, DEFAULT_LEVEL )

-- draw lower line
draw_line( verts_dn, tp.material, VideoAreaPH.name, nil, nil, nil, h_clip_relations.COMPARE, DEFAULT_LEVEL )

-- draw caption																		
for i = 1, #caption do
	addText( string.sub(caption, i, i).."\n", {pos_caption[1], pos_caption[2] - 30 }, tp, nil, nil, nil, nil, VideoAreaPH.name, h_clip_relations.INCREASE_LEVEL, DEFAULT_LEVEL )
	pos_caption = {pos_caption[1], pos_caption[2] - tp.height  + 2}
end
