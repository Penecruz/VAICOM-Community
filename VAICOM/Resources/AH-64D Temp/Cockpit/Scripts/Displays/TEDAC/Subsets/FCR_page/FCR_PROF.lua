...
local VideoAreaPH = addPlaceholder("VideoAreaCenterPHtpm", {-30, 45}, OnOffPagePROF.name, nil)
...
local function createVertsForBackground(verts_r, verts_dn)
   ...
   local offsetX, offsetLine = 3, info_box_line_width / 2
	...
	verts_out[4][2][2] = display_size_pix_05 - 450
...
-- draw caption																		
for i = 1, #caption do
	addText( string.sub(caption, i, i).."\n", {pos_caption[1], pos_caption[2] + 50}, tp, nil, nil, nil, nil, VideoAreaPH.name, h_clip_relations.REWRITE_LEVEL, DEFAULT_LEVEL )
