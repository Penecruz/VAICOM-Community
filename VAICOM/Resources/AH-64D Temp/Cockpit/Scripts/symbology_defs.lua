...
function setPlaceholderCommonProperties(placeholder, name, pos, parent, controllers)
	...
	pos							= pos or {0, 0, 0}
	placeholder.init_pos		= {pos[1], pos[2], pos[3] or 0}
...
function addStrokeText(name, value, stringdef, align, pos, parent, controllers, formats)
	...
	setSymbolCommonProperties(txt, name, pos, parent, controllers, current_font)
...
function addMaskedFatDashedLine(name, length, stroke, gap, width, pos, rot, parent, controllers, material)
	local segLength			= stroke + gap
	local numOfWholePairs	= math.floor(length / segLength)
	local reminder			= length - numOfWholePairs * segLength
	...
	local function addSegment(num, len)
		local pos0 = { pos[1] + dx * num, pos[2] + dy * num}
		addMaskedFatLine(name.." _seg "..num, len, width, pos0, rot, parent, controllers, material)
	end
...
function createMask(name, vertices, indices, pos, parent, controllers, material)
...
function add_render_target(controllers)
	local width = GetHalfWidth() * 2
	local height = GetHalfHeight() * 2
	local	render_video					= CreateElement "ceTexPoly"
			render_video.name				= "VideoSignal"
			render_video.vertices			= buildBoxVerts(width, height, "CenterCenter")
			render_video.indices			= default_box_indices
			render_video.tex_coords			= {{0, 1},{0, 0},{1, 0},{1, 1}}
			render_video.input_space_SRGB	= true
			---------------------------------------------------------------------------------------
			render_video.material			= "render_target_"..string.format("%d", GetRenderTarget() + 1)
			---------------------------------------------------------------------------------------
			if controllers then 
				render_video.controllers = controllers
			end
			Add(render_video)
	return  render_video
end
