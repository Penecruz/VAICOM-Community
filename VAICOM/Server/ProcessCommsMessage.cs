...
namespace VAICOM
{
    namespace Servers
    {
        ...
        public static partial class Server
        {
            ...
            public static void ProcessCommsMessageContent(ServerCommsMessage message)
            {
                ...
                else // Start of message
                {
                    ...
                    if (message.eventkey.StartsWith("wMsgWSO"))
                    {
                        ...
                        {
                            Log.Write($"Unknown WSO Command: {message.eventkey}", Colors.Warning);
                        }

                        return; // Exit after processing WSO message
                    }

                    bool isAutoGenerated = AutoGeneratedMessage(message.eventid);
                    bool playerIsSender = message.pMsgSender != null && message.pMsgSender.id_.Equals(State.currentstate.playerunitid);
                    bool playerIsReceiver = message.pMsgReceiver != null && message.pMsgReceiver.id_.Equals(State.currentstate.playerunitid);
                    bool suppressAuto = State.activeconfig.CarrierSuppressAuto && isAutoGenerated && (playerIsSender || playerIsReceiver);
                    if (suppressAuto)
                    {
                        return;
                    }

                    bool auto = !State.activeconfig.CarrierSuppressAuto && isAutoGenerated;

                    if (playerIsSender || auto)
                    {
                        if (auto)
                        {
