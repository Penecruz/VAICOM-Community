        ...
        public partial class DcsClient
        {
            public static partial class Message
            {
                ...
                private static bool TryBuildRioEgiDirectWaypointSequence(int waypointNumber, out string validationError)
                {
                    ...
                    State.currentmessage.extsequence.Add(Extensions.RIO.DeviceActionsLibrary.RIO.Atom_J_PROXY_BU_DIRECT_TO_SELECTED_DELAYED(120));
                ...
                private static bool TryGetEgiWaypointFromSegments(out int waypointNumber)
                {
                    ...
                    int parsed;
                    if (Int32.TryParse(State.Proxy.Utility.ParseTokens("{CMDSEGMENT:1}"), out parsed) && parsed > 0)
                    {
                        waypointNumber = parsed;
                        return true;
                    }

                    if (Int32.TryParse(State.Proxy.Utility.ParseTokens("{CMDSEGMENT:2}"), out parsed) && parsed > 0)
                    {
                        waypointNumber = parsed;
                        return true;
                    }

                    if (Int32.TryParse(State.Proxy.Utility.ParseTokens("{CMDSEGMENT:3}"), out parsed) && parsed > 0)
                    {
                        waypointNumber = parsed;
                        return true;
                    }
